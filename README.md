...
- 命令说明、输入提示、错误信息、切换按钮和线程标识支持中文与英文，并跟随 DeepSeek Harness 的语言设置实时切换。

侧会话从执行命令时主会话最近一个已完成边界分叉，继承模型、推理配置、工具、沙箱与审批策略。它支持多轮追问，不阻塞主会话，也不会把任何侧会话消息写回主会话上下文。

## 环境要求

需要 DeepSeek Harness `>= 0.1.0-rc.6`，并包含 `ctx.subagents.startEphemeralContinuable()` 及其句柄的 `relabel()` 宿主 API；插件不会通过安装脚本修改宿主。开发时不能直接使用尚未包含这些 API 的全局 `rc.6` 产物。
