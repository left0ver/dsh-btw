...
- `Ctrl+/`：在主会话和活动侧会话之间往返切换，不会在切回主会话时关闭侧会话。BTW 复用原生会话视图、输入框和工具消息展示。
- 输入框下方会标明当前是“主线程”还是“子线程”。
- 当前标签页会保留临时的导航关系，因此刷新页面后仍可切换并显示线程标识；该关系不会写入 Harness 会话 lineage。
...
- rc.6 会全局持久化所有公开 Agent，因此侧会话活动期间也会暂存日志。退出时插件先 flush、释放 `AgentHandle`，再只删除该随机会话 ID 对应的 JSONL artifact；不会递归删除目录。
